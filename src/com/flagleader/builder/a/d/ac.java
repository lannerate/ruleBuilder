...
class ac
  implements Listener
{
  ac(W paramW, Item[] paramArrayOfItem)
  {
  }
    ...
    if (str1 != null)
    {
      ...
      try
      {
        ...
        if (localWizardDialog.open() == 0)
        {
          ...
          ((PageFieldModel)this.b[0].getData()).setPropertyValue("valueField", "id");
          ((PageFieldModel)this.b[0].getData()).setPropertyValue("textField", "text");
          if (((PageFieldModel)this.b[0].getData()).getJavaType().isString())
            ((PageFieldModel)this.b[0].getData()).setPropertyValue("multiple", "true");
